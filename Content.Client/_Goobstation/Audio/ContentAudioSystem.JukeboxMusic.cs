...
{
    private const float JukeboxMusicFadeTime = 10f;

    private void InitializeJukeboxMusic()
    {
        Subs.CVar(_configManager, CCVars.AmbientMusicVolume, JukeboxCVarChanged, true);
        ...
        SetupJukeboxSounds();
        SubscribeLocalEvent<PrototypesReloadedEventArgs>(OnJukeboxProtoReload);
        _state.OnJukeboxStateChanged += OnJukeboxStateChange;
        // On round end summary OR lobby cut audio.
        SubscribeNetworkEvent<RoundEndMessageEvent>(OnRoundEndMessage);
    }

    private void JukeboxCVarChanged(float obj)
    {
